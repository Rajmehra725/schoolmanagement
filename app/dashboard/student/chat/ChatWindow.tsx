...
export default function ChatBox({ userId, selectedUserId, selectedUserName, chatId }: ChatBoxProps) {
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      updateDoc(typingRef, { typing: input.length > 0 });
    }, 100);
  ...
  const markDelivered = async (msgs: Message[]) => {
    for (const msg of msgs) {
      if (msg.receiverId === userId && msg.status === 'sent') {
        await updateDoc(doc(db, 'chats', chatId, 'messages', msg.id), { status: 'delivered' });
      }
    }
  };
  ...
  const handleReaction = async (index: number, emoji: string) => {
    ...
    await updateDoc(doc(db, 'chats', chatId, 'messages', msg.id), { reaction: emoji });
  };
  ...
  const sendMessage = async () => {
    ...
    try {
      if (editMsgId) {
        await updateDoc(doc(db, 'chats', chatId, 'messages', editMsgId), { text: input });
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto p-4 bg-gray-50 space-y-3 custom-scroll relative">
              ...
              return (
                <motion.div
                  ...
                  className={`group relative p-3 max-w-[75%] rounded-xl shadow-sm text-sm break-words ${
                    isOwn ? 'ml-auto bg-blue-100 text-right text-gray-800' : 'bg-white text-left text-gray-900'
                  }`}
                >
                  <div>{msg.text}</div>

                  {msg.reaction && <div className="text-xl mt-1"><span>{msg.reaction}</span></div>}

                  <div className="absolute -top-8 ${isOwn ? 'right-0' : 'left-0'} bg-white p-1 rounded-lg shadow flex gap-1">
                    {emojiOptions.map((emoji) => (
                      <button key={emoji} className="hover:scale-110 transition text-lg" onClick={() => handleReaction(index, emoji)}>
                        {emoji}
                      </button>
                    ))}
                  </div>
                  ...
                  <div className="text-[10px] text-gray-500 mt-1 flex justify-between items-center gap-2">
                    ...
                    {isOwn && (
                      <div className="flex gap-2 text-xs font-semibold">
                        {msg.status && <span className={
                          msg.status === 'seen' ? 'text-green-500' :
                          msg.status === 'delivered' ? 'text-blue-500' : 'text-gray-400'
                        }>{msg.status}</span>}
                        <button onClick={() => handleEdit(msg)} className="text-yellow-500 hover:underline">Edit</button>
        ...
        {typing && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex items-center text-gray-500 italic text-sm gap-1 mt-2">
      ...
      <div className="p-3 bg-white border-t flex gap-2 max-sm:flex-col max-sm:items-stretch">
        <input
          ...
          onKeyDown={(e) => {
            if (e.key === 'Enter' && (e.ctrlKey || !e.shiftKey)) sendMessage();
          }}
        />
        <button
          disabled={sending}
          className={`${sending ? 'bg-blue-300 cursor-not-allowed' : 'bg-blue-500 hover:bg-blue-600'} text-white px-4 py-2 rounded-full text-sm font-semibold`}
          onClick={sendMessage}
        >
          {editMsgId ? 'Update' : sending ? 'Sending...' : 'Send'}
        </button>
      </div>
      <div className="text-xs text-gray-400 px-4 pb-2 text-right">{input.length}/200</div>
    </div>
  );
}
